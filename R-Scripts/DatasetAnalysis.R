...
for (kv in 1:8 * 4) {
  # solo per alpha = 0.10
  NM1 <- filter(dati1, dati1$k == kv & dati1$model == nullModel1) # tutte le misure per lo specifico NM e valore di alpha
  AM1 <- filter(dati1, dati1$k == kv & dati1$model == altModels1[1]) # tutte le misure per uno specifico AM e valore di alpha

  NM2 <- filter(dati2, dati2$k == kv & dati2$model == nullModel2) # tutte le misure per lo specifico NM e valore di alpha
  AM2 <- filter(dati2, dati2$k == kv & dati2$model == altModels2[1]) # tutte le misure per uno specifico AM e valore di alpha
